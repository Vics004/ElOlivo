...
namespace ElOlivo.Servicios
{
    public class SupabaseService
    {
        ...
        // Agregar estos métodos a SupabaseService.cs

        private readonly string _bucketFiles = "files";

        public async Task<string> SubirArchivoFiles(IFormFile archivo, string nombreArchivo)
        {
            try
            {
                _logger.LogInformation($"Iniciando subida de archivo al bucket files: {nombreArchivo}");

                // Verificar que el bucket existe, si no crearlo
                var buckets = await _supabase.Storage.ListBuckets();
                _logger.LogInformation($"Buckets encontrados: {buckets.Count}");

                if (!buckets.Any(b => b.Name == _bucketFiles))
                {
                    _logger.LogInformation($"Creando bucket: {_bucketFiles}");
                    await _supabase.Storage.CreateBucket(_bucketFiles, new Supabase.Storage.BucketUpsertOptions
                    {
                        Public = true
                    });
                }
            ...
            {
                _logger.LogError(ex, "Error al subir archivo al bucket files");
                throw new Exception($"Error al subir archivo: {ex.Message}");
            }
        }

        public async Task<bool> EliminarArchivoFiles(string nombreArchivo)
        {
            try
            {
                _logger.LogInformation($"Eliminando archivo del bucket files: {nombreArchivo}");
                await _supabase.Storage
                    .From(_bucketFiles)
                    .Remove(new List<string> { nombreArchivo });
            ...
            {
                _logger.LogError(ex, "Error al eliminar archivo del bucket files");
                return false;
            }
        }
    }
